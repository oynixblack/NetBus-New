...
include 'connection.php';
    ...
    if(isset($_GET['reset'])) { 
        ...
        $verifyQuery = mysqli_query($conn,"SELECT * FROM login WHERE code = '{$_GET['reset']}'");
        ...
        if(mysqli_num_rows($verifyQuery)>0) {
            if(isset($_POST['change'])) {
                ...
                $log="SELECT * FROM `login` WHERE username='{$email}'";
                $r_log=mysqli_query($conn,$log);
                ...
                if(mysqli_num_rows($r_log)==0)
                {
                    ...
                    $changeQuery = mysqli_query($conn,"UPDATE login SET password='{$password}', code=''  WHERE code='{$_GET['reset']}'");
                /* echo $changeQuery; */
                if($changeQuery) {
        ...
        header("Location: forgetpasslogin.php");
    }  
...
<body>
    ...
    <div class="container p-3 border border-5 rounded-3"  style="width: 50%">
        ...
        <form action="" method="post" id="signup">
            ...
            <div class="form-field">
                <label for="c-password">Confirm New Password:</label><br>
