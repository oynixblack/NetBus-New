...
if (isset($_GET['reference'])) {
  ...
  curl_setopt_array($pay, array(
    ...
    CURLOPT_HTTPHEADER => [
      ...
      "authorization: Bearer sk_test_71c2dc8c8c569dcfe1211f855d4bc749f5978276",
  ...
  if ($response) {
    ...
    if (array_key_exists('data', $result) && array_key_exists('status', $result['data']) && ($result['data']['status'] === 'success') && ($result['data']['requested_amount'] === intval($amount))) {
      ...
      $ins = $conn->query("INSERT INTO payment (passenger_id, schedule_id, amount, ref, date) VALUES ('loginid','$schedule_id', '$paid', '$reference', '$date')");
      $code = genCode($schedule_id, loginid, $class);
      $seat = genSeat($schedule_id, $class, $number);
      $payment_id = $conn->insert_id;
      if ($payment_id > 0) {

        $conn->query("INSERT INTO booked (payment_id, schedule_id, user_id, code, class, no, date, seat) VALUES ('$payment_id','$schedule_id', 'loginid', '$code', '$class', '$number', '$date' , '$seat')");
